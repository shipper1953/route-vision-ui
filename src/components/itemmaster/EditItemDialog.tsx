...
export const EditItemDialog = ({ item, isOpen, onClose, onUpdate }: EditItemDialogProps) => {
  ...
  useEffect(() => {
    if (item) {
      setFormData({
        ...
        eachesPerInnerpack: (item.uomEaches?.innerpack || 1).toString(),
        eachesPerCase: (item.uomEaches?.case || 1).toString(),
        eachesPerPallet: (item.uomEaches?.pallet || 1).toString()
      });
    }
  }, [item]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!item || !formData.sku || !formData.name || !formData.length || !formData.width || !formData.height || !formData.weight) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Update item details and dimensions
          </DialogDescription>
        </DialogHeader>
