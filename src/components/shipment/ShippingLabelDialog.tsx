...
export const ShippingLabelDialog = ({
  ...
  const [labelUrl, setLabelUrl] = useState<string | undefined>(initialLabelUrl);
  const [refreshing, setRefreshing] = useState(false);
  const [autoRefreshFailed, setAutoRefreshFailed] = useState(false);
  const [autoAttempts, setAutoAttempts] = useState(0);

  const AUTO_REFRESH_INTERVAL_MS = 4000;
  const AUTO_REFRESH_MAX_ATTEMPTS = 10;

  // Sync local state when props change (e.g. dialog reopened with new shipment)
  useEffect(() => {
    setLabelUrl(initialLabelUrl);
    setAutoRefreshFailed(false);
    setAutoAttempts(0);
  }, [initialLabelUrl, shipmentId]);

  const isPending = !labelUrl && !!shipmentId && (labelPending || provider === 'easyship');

  // Returns 'ready' | 'failed' | 'pending' | 'error' so callers (manual + auto) can react.
  const refreshLabelOnce = async (
    opts: { silent?: boolean } = {},
  ): Promise<'ready' | 'failed' | 'pending' | 'error'> => {
    const { silent = false } = opts;
    if (!shipmentId) return 'error';
    const numericId = Number(shipmentId);
    if (Number.isNaN(numericId)) {
      if (!silent) toast.error('Cannot refresh: invalid shipment reference');
      return 'error';
    }
    setRefreshing(true);
    try {
      const { data, error } = await supabase.functions.invoke('refresh-easyship-label', {
        body: { shipmentId: numericId },
      });
      if (error) throw error;
      if (data?.status === 'ready' && data.labelUrl) {
        setLabelUrl(data.labelUrl);
        if (!silent) toast.success('Label is ready!');
        else toast.success('Label is ready!');
        return 'ready';
      }
      if (data?.status === 'failed') {
        if (!silent) toast.error(data.message || 'Carrier reported the label generation failed');
        return 'failed';
      }
      if (!silent) {
        toast.info('Label is still being generated by the carrier. Please try again in a moment.');
      }
      return 'pending';
    } catch (e: any) {
      console.error('Refresh label failed:', e);
      if (!silent) toast.error(e?.message || 'Failed to refresh label');
      return 'error';
    } finally {
      setRefreshing(false);
    }
  ...
  const handleRefreshLabel = async () => {
    // Manual refresh resets the auto-poll budget so the timer resumes.
    setAutoRefreshFailed(false);
    setAutoAttempts(0);
    await refreshLabelOnce();
  };

  // Auto-refresh loop while the dialog is open, label is pending, and we still have attempts left.
  useEffect(() => {
    if (!isOpen || !isPending || autoRefreshFailed) return;
    if (autoAttempts >= AUTO_REFRESH_MAX_ATTEMPTS) return;

    let cancelled = false;
    const timer = window.setTimeout(async () => {
      if (cancelled) return;
      const result = await refreshLabelOnce({ silent: true });
      if (cancelled) return;
      if (result === 'ready') return; // effect will re-run; isPending becomes false
  ...
  }, [isOpen, isPending, autoAttempts, autoRefreshFailed, shipmentId]);
