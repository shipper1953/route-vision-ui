...
export const ShippingLabelDialog = ({
  ...
  const handleRefreshLabel = async () => {
    ...
    if (Number.isNaN(numericId)) {
      toast.error('Cannot refresh: invalid shipment reference');
      return;
    }
    setRefreshing(true);
    try {
      ...
      if (data?.status === 'ready' && data.labelUrl) {
        ...
        toast.error(data.message || 'Carrier reported the label generation failed');
      } else {
      ...
      toast.error(e?.message || 'Failed to refresh label');
    } finally {
      setRefreshing(false);
    }
  };
