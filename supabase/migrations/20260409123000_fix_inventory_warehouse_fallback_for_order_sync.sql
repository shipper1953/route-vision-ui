-- Fallback allocation/deduction to any warehouse row for the item when order warehouse mapping differs
...
BEGIN
  IF p_company_id IS NULL OR p_items IS NULL THEN
    RETURN;
  END IF;
...
BEGIN
  ...
  LOOP
    ...
    -- Fallback when no allocated units were found in the order's warehouse
...
-- Recalculate allocations so existing open orders are reflected after fallback changes.
UPDATE public.inventory_levels
SET
