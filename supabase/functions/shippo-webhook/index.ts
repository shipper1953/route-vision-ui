...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      const trackingEvents = trackingData.tracking_history.map((
        event: any,
      ) => ({
        shipment_id: shipment.id,
        provider: "shippo",
        event_type: mapShippoStatusToEventType(
          event.status,
      ...
      console.log(
        `Stored ${trackingEvents.length} Shippo tracking events for shipment ${shipment.id}`,
      );
    }
    ...
    if (
      ...
      console.log(
        `Updated shipment ${shipment.id} with Shippo delivery information`,
      );
...
function mapShippoStatusToEventType(
  status: string,
  statusDetail: string,
): string {
  ...
  if (statusUpper === "DELIVERED" || detailLower.includes("delivered")) {
    return "delivered";
  }
  ...
  if (statusUpper === "RETURNED" || detailLower.includes("return")) {
    return "returned";
  }
  if (statusUpper === "FAILURE" || statusUpper === "UNKNOWN") {
    return "exception";
  }
