...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      webhookEvent.object !== "Event" || !webhookEvent.result ||
      webhookEvent.result.object !== "Tracker"
    ) {
    ...
    if (!shipments || shipments.length === 0) {
      console.log(
        `No shipment found for tracking code: ${trackingCode} or shipment ID: ${shipmentId}`,
      );
    ...
    if (tracker.tracking_details && tracker.tracking_details.length > 0) {
      ...
      console.log(
        `Stored ${trackingEvents.length} tracking events for shipment ${shipment.id}`,
      );
    }
    ...
    if (tracker.est_delivery_date) {
      ...
      console.log(
        `Setting estimated delivery date from EasyPost: ${estimatedDeliveryDate}`,
      );
    }
    ...
    if (
      ...
      tracker.status_detail === "arrived_at_destination"
    ) {
      isDelivered = true;
    }
    ...
    const deliveredEvent = tracker.tracking_details?.find((detail) =>
      ...
      detail.status === "out_for_delivery" &&
        detail.message?.toLowerCase().includes("delivered")
    );
    ...
    if (
      ...
      console.log(
        `Updated shipment ${shipment.id} with delivery information from EasyPost webhook`,
      );
      ...
      if (Object.keys(orderUpdate).length > 0) {
        ...
        for (const jo of junctionOrders || []) {
          ...
          console.log(
            `✅ Propagated delivery info to order ${jo.order_id} via junction`,
          );
        }
      }
      ...
      if (
        status === "out_for_delivery" && shipment.status !== "out_for_delivery"
      ) {
...
function mapStatusToEventType(status: string, statusDetail: string): string {
  ...
  if (statusLower === "delivered" || detailLower.includes("delivered")) {
    return "delivered";
  }
  if (
    statusLower === "out_for_delivery" ||
    detailLower.includes("out for delivery")
  ) return "out_for_delivery";
  if (statusLower === "in_transit") return "in_transit";
  if (statusLower === "pre_transit") return "pre_transit";
  if (statusLower === "returned" || detailLower.includes("return")) {
    return "returned";
  }
...
async function triggerNotification(
  ...
  try {
    ...
    if (orderShipment?.orders) {
      ...
      console.log(
        `Triggered ${notificationType} notification for shipment ${shipmentId}`,
      );
    }
  } catch (error) {
