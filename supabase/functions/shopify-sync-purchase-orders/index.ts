...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { companyId, storeId, dateRangeDays = 90, status } = await req
      .json() as PurchaseOrderSyncRequest;

    console.log(
      `[PO Sync] Starting sync for company ${companyId}, store ${storeId}`,
    );
    ...
    while (hasMore) {
      ...
      if (!response.ok) {
        ...
        console.error(
          `[PO Sync] Shopify API error - Status: ${response.status} ${response.statusText}`,
        );
    ...
    for (const shopifyPO of purchaseOrders) {
      try {
        ...
        const poData = {
          ...
          expected_date: shopifyPO.expected_at
            ? new Date(shopifyPO.expected_at).toISOString().split("T")[0]
            : null,
        ...
        if (shopifyPO.line_items && Array.isArray(shopifyPO.line_items)) {
          for (const lineItem of shopifyPO.line_items) {
            ...
            const { data: matchedItem } = await supabase
              ...
              .eq("company_id", companyId)
              .or(
                `sku.eq.${lineItem.sku},shopify_variant_id.eq.${lineItem.variant_id}`,
              )
              .limit(1)
              .single();
        ...
        console.error(
          `[PO Sync] Error processing PO ${shopifyPO.name}:`,
          error,
        );
        errors++;
      }
    ...
    console.log(
      `[PO Sync] Complete: ${created} created, ${updated} updated, ${errors} errors`,
    );
