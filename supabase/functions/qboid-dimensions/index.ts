...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-qboid-token",
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      const dimensions = {
        ...
        weight: qboidData.weight
          ? parseFloat((qboidData.weight / 453.592).toFixed(2))
          : 0,
      };
      ...
      if (qboidData.barcode) {
        ...
        return new Response(
          JSON.stringify({
            success: true,
            message:
              "Dimensions captured. Please scan barcode to link to item.",
            data: { dimensions },
          }),
          {
            headers: corsHeaders,
    ...
    return new Response(
      JSON.stringify({
        error:
          "Invalid request. Only POST requests with application/json content-type are supported.",
      }),
      {
        headers: corsHeaders,
        status: 405,
