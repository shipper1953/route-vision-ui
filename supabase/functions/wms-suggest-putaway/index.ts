  ...
  try {
    ...
    const scoredLocations = locations.map((location) => {
      ...
      if (location.capacity_cubic_ft && itemDimensions) {
        const itemVolume = itemDimensions.length * itemDimensions.width *
          itemDimensions.height / 1728;
        const remainingCapacity = location.capacity_cubic_ft -
          (currentStock * itemVolume);
...
function generateReason(location: any): string {
  if (location.score >= 150) {
    return "Optimal location - high capacity, low congestion";
  }
