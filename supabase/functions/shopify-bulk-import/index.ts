...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Importing orders from ${startDate.toISOString()} to ${endDate.toISOString()}`,
    );
    ...
    if (!response.ok) {
      ...
      console.error(
        `Shopify API error - Status: ${response.status} ${response.statusText}`,
      );
    ...
    for (const shopifyOrder of orders) {
      try {
        ...
        if (variantIdSet.size > 0) {
          ...
          (matches || []).forEach((match: any) => {
            if (match.shopify_variant_id) {
              matchedByVariantId.set(match.shopify_variant_id, match);
            }
          });
        }
        ...
        if (productIdSet.size > 0) {
          ...
          (matches || []).forEach((match: any) => {
            if (match.shopify_product_id) {
              matchedByProductId.set(match.shopify_product_id, match);
            }
          });
        }

        const items = rawItems.map((item: any) => {
          const matchedItem = (item.variant_id
            ? matchedByVariantId.get(item.variant_id)
            : null) ||
            (item.product_id
              ? matchedByProductId.get(item.product_id)
              : null) ||
        ...
        const hasUnmatchedItems = items.some((item: any) =>
