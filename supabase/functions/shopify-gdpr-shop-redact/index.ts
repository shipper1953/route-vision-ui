  ...
  try {
    ...
    await supabase.from("analytics_events").insert({
      ...
      payload: {
        ...
        retention_note:
          "Order and financial data retained for 7 years per legal requirements",
      },
    });
    ...
    return new Response(
      JSON.stringify({
        ...
        note:
          "Shopify integration data has been removed. Historical orders and shipments are retained for accounting purposes.",
      }),
      {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
