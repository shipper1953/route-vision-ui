...
async function shopifyRest(
  ...
  if (!response.ok) {
    throw new Error(
      `REST HTTP error: ${response.status} ${response.statusText}`,
    );
  }
...
serve(async (req) => {
  ...
  const { shipmentId, status, trackingNumber, trackingUrl, carrier, service } =
    requestBody;
  ...
  try {
    ...
    if (storeId) {
      ...
      console.log(
        "Store lookup result:",
        storeData ? `Found: ${storeData.store_url}` : "Not found",
        "Error:",
        storeError?.message,
      );
      store = storeData;
    ...
    if (!store?.access_token) {
      console.log(
        "Store not found by ID, trying company_id fallback...",
        order.company_id,
      );
    ...
    const shopifyOrderId = mapping.shopify_order_id.toString().replace(
      "gid://shopify/Order/",
      "",
    );
    ...
    const priorFulfillmentIdForShipment =
      priorSyncForShipment?.metadata?.fulfillment_id || null;
    ...
    if (priorFulfillmentIdForShipment) {
      ...
      trackingOnlyTarget = existingShopifyFulfillments.find(
        (fulfillment: any) => {
    ...
    if (trackingOnlyTarget) {
      console.log(
        `ℹ️ Existing Shopify fulfillment for shipment ${shipmentId}; updating tracking only:`,
        trackingOnlyTarget.id,
      );
      const trackingOnlyResult = await updateFulfillmentTrackingInfo(
        store.store_url,
        store.access_token,
        trackingOnlyTarget.admin_graphql_api_id ||
          `gid://shopify/Fulfillment/${trackingOnlyTarget.id}`,
        trackingNumber,
      );
      ...
      await supabase
        ...
        .update({
          ...
          metadata: {
            ...mapping.metadata,
            last_fulfillment_id: trackingOnlyTarget.admin_graphql_api_id ||
              `gid://shopify/Fulfillment/${trackingOnlyTarget.id}`,
      ...
      await supabase.from("shopify_sync_logs").insert({
        ...
        metadata: {
          ...
          fulfillment_id: trackingOnlyTarget.admin_graphql_api_id ||
            `gid://shopify/Fulfillment/${trackingOnlyTarget.id}`,
      ...
      return new Response(
        JSON.stringify({
          success: true,
          fulfillmentId: trackingOnlyTarget.admin_graphql_api_id ||
            `gid://shopify/Fulfillment/${trackingOnlyTarget.id}`,
...
async function fetchFulfillmentOrdersFromShopify(
  ...
  console.log(
    `📡 Fetching fulfillment orders from Shopify for order ${shopifyOrderId}`,
  );
  ...
  console.log(
    `Found ${fulfillmentOrders.length} fulfillment orders from Shopify API`,
  );
  ...
  console.log(
    `Open fulfillment orders: ${openOrders.length} (statuses: ${
      fulfillmentOrders.map((fo: any) => fo.status).join(", ")
    })`,
  );
...
async function cancelExistingFulfillment(
  ...
  console.log(
    "✅ Existing fulfillment canceled:",
    result?.fulfillmentCancel?.fulfillment?.id,
  );
...
async function updateFulfillmentTrackingInfo(
  ...
  if (errors.length > 0) {
    throw new Error(
      `Tracking update errors: ${errors.map((e: any) => e.message).join(", ")}`,
    );
  }
...
async function handleFulfillmentServiceFlow(
  ...
  if (localFOs && localFOs.length > 0) {
    ...
    console.log(
      "No local fulfillment orders found - fetching from Shopify API...",
    );
    ...
    if (shopifyFOs.length > 0) {
      ...
      console.log(
        "✅ Built fulfillment order from Shopify API:",
  ...
  const normalizeText = (v: any) => v ? v.toString().toLowerCase().trim() : "";

  if (
    orderShipment.package_info?.items &&
    Array.isArray(orderShipment.package_info.items)
  ) {
    const shippedItems = orderShipment.package_info.items;

    shippedItems.forEach((si: any) =>
      shippedItemsSummary.push({
        sku: si.sku,
        name: si.name,
        quantity: si.quantity,
        variant_id: si.shopifyVariantId,
      })
    );
    lineItems.forEach((li: any) =>
      foItemsSummary.push({
        ...
        remaining: li.remainingQuantity ?? li.fulfillable_quantity ??
          li.quantity,
      })
    );
    ...
    console.log(
      `Available line items with remaining quantity: ${availableLineItems.length} of ${lineItems.length}`,
    );
    ...
    for (const foLineItem of availableLineItems) {
      ...
      const shippedItem = shippedItems.find((si: any) => {
        ...
        if (si.shopifyVariantId && foLineItem.variant_id) {
          if (
            normalizeVariantGid(si.shopifyVariantId) ===
              normalizeVariantGid(foLineItem.variant_id)
          ) {
      ...
      if (shippedItem && matchedBy) {
        const remaining = foLineItem.fulfillable_quantity ??
          foLineItem.remainingQuantity ?? foLineItem.quantity ?? 0;
        const qtyToFulfill = Math.max(
          0,
          Math.min(shippedItem.quantity, remaining),
        );
        if (qtyToFulfill <= 0) {
          console.warn(
            `⚠️ Skipping line item with zero remaining capacity:`,
            foLineItem,
          );
          continue;
        }
        ...
        const lineItemGid = foLineItem.fulfillment_order_line_item_gid ||
          (foLineItem.id.startsWith("gid://")
            ? foLineItem.id
            : `gid://shopify/FulfillmentOrderLineItem/${foLineItem.id}`);
        ...
        console.log(
          `✅ Will fulfill: ${
            foLineItem.sku || foLineItem.name
          } - ${qtyToFulfill} units (matched by ${matchedBy})`,
        );
      } else {
    ...
    if (shippedItems.length > 0 && fulfillmentOrderLineItems.length === 0) {
      const errorMsg =
        "Shipped items did not match any fulfillment-order line items; aborting to avoid blank fulfillment";
  ...
  const fulfillmentOrderGid =
    fulfillmentOrderData.fulfillment_order_id.startsWith("gid://")
      ? fulfillmentOrderData.fulfillment_order_id
  ...
  console.log(
    "Creating fulfillment with variables:",
  ...
  await supabase
    ...
    .update({
      ...
      metadata: {
        ...
        tracking_info: trackingFulfillment?.trackingInfo ??
...
async function handleLegacyFlow(
    ...
    for (const foLineItem of foLineItems) {
      ...
      const shippedItem = shippedItems.find((si: any) => {
        if (si.shopifyVariantId && foLineItem.lineItem?.variant?.id) {
          if (
            normalizeVariantGid(si.shopifyVariantId) ===
              normalizeVariantGid(foLineItem.lineItem.variant.id)
          ) {
            matchedBy = "variant_id";
            return true;
          }
        ...
        if (si.sku && foLineItem.lineItem?.sku) {
          if (
            normalizeText(si.sku) === normalizeText(foLineItem.lineItem.sku)
          ) {
            matchedBy = "sku";
            return true;
          }
        ...
        if (si.name && foLineItem.lineItem?.name) {
          if (
            normalizeText(si.name) === normalizeText(foLineItem.lineItem.name)
          ) {
            matchedBy = "name";
            return true;
          }
      ...
      if (shippedItem && matchedBy) {
        const qty = Math.max(
          0,
          Math.min(shippedItem.quantity, foLineItem.remainingQuantity),
        );
        if (qty <= 0) continue;
  ...
  console.log(
    "Creating fulfillment (legacy GraphQL):",
    JSON.stringify(variables, null, 2),
  );
