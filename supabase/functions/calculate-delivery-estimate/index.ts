...
Deno.serve(async (req) => {
  ...
  try {
    const { warehouseId, destinationZip, items }: EstimateRequest = await req
      .json();
    ...
    const largestDim = Math.max(
      ...items.map((i) => Math.max(i.length, i.width, i.height)),
    );
    ...
    const latestDelivery = deliveryEstimates.length > 0
      ? calculateDeliveryDate(
        Math.max(...deliveryEstimates.map((e: any) => e.days)),
      )
      : null;
    ...
    return new Response(
      JSON.stringify({ error: "Failed to calculate estimate" }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
