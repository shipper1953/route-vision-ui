...
serve(async (req) => {
  ...
  try {
    ...
    if (
      !poLine || poLine.purchase_orders.company_id !== userProfile.company_id
    ) {
    ...
    if (!qcRequired && condition === "good") {
        ...
        .then(async ({ data: existingLevel, error: selectError }) => {
          ...
          if (existingLevel) {
            ...
            return await supabase
              .from("inventory_levels")
              .update({
                quantity_on_hand: existingLevel.quantity_on_hand +
                  quantityReceived,
                quantity_available: existingLevel.quantity_available +
                  quantityReceived,
                condition,
      ...
      if (transaction) {
        try {
          ...
          console.log(
            "Initiated Shopify sync for transaction:",
            transaction.id,
          );
        } catch (syncError) {
    ...
    if (
      poLineCheck &&
      poLineCheck.quantity_received >= poLineCheck.quantity_ordered
    ) {
      ...
      if (allReceived) {
        ...
        console.log(
          "PO fully received, status updated to received:",
          poLineCheck.po_id,
        );
      }
    }
