  ...
  try {
    ...
    if (allShipments && allShipments.length > 0) {
      ...
      if (orderIds.length > 0) {
        ...
        (cartonizations || []).forEach((c) =>
          cartonizationByOrderId.set(Number(c.order_id), c)
        );
      }
    ...
    for (const shipment of matchedShipments) {
      ...
      const newUtil = Number(
        shipment.recommended_utilization ?? utilizationPct(items, recDims),
      );
      const improvement = newUtil - actualUtil;
      const recommendedSku = recommendedBox.sku || recommendedBox.id ||
        recommendedBox.name;
      ...
      if (
        ...
        recommendedSku !== shipment.actual_package_sku
      ) {
        hasOpportunity = true;
