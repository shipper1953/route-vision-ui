...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      shopifySync = await pushItemToShopify(
        supabaseClient,
  ...
  try {
    const res = await fetch(
      `https://${storeUrl}/admin/api/2025-01/graphql.json`,
      {
        ...
        body: JSON.stringify({
          query:
            `query($id: ID!) { fulfillmentService(id: $id) { id location { id name } } }`,
          variables: { id: fsId },
        }),
      },
    );
...
> {
  ...
  const { data: item, error: itemErr } = await admin
    .from("items")
    .select(
      "id, sku, shopify_variant_gid, shopify_store_id, company_id, is_active",
    )
    .eq("id", itemId)
    .maybeSingle();
  ...
  if (item.company_id !== companyId) {
    return { attempted: false, error: "Company mismatch" };
  }
  ...
  if (storeErr || !store) {
    return { attempted: false, error: "Shopify store not found or inactive" };
  }
  ...
  if (invErr) {
    return { attempted: true, ok: false, error: invErr.message, sku: item.sku };
  }
  ...
  const fsLocationId = await resolveFulfillmentServiceLocationId(
    store,
    storeUrl,
  );
  if (!fsLocationId) {
    return {
      attempted: true,
      ok: false,
      error: "No Ship Tornado fulfillment-service location resolved",
      sku: item.sku,
    };
  }
  ...
  if (!variantRes.ok) {
    return {
      attempted: true,
      ok: false,
      error: `Variant lookup HTTP ${variantRes.status}`,
      sku: item.sku,
    };
  }
  ...
  if (variantJson.errors?.length) {
    return {
      attempted: true,
      ok: false,
      error: `Variant GraphQL error: ${JSON.stringify(variantJson.errors)}`,
      sku: item.sku,
    };
  }
  ...
  if (!inventoryItem || !targetEdge) {
    return {
      ...
      ok: false,
      error:
        `Ship Tornado location ${fsLocationId} not present on variant in Shopify`,
      sku: item.sku,
    };
  }
  ...
  console.log(
    `✅ Pushed ${item.sku} → ${totalAvailable} at location ${fsLocationId}`,
  );
  return {
    attempted: true,
    ok: true,
    sku: item.sku,
    pushedQty: totalAvailable,
  };
}
