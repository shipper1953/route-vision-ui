...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-shopify-shop-domain",
};
...
interface FulfillmentServiceCallback {
  kind:
    ...
    | "FULFILLMENT_REQUEST_HOLD"
    | "FULFILLMENT_REQUEST_RELEASE_HOLD";
}
...
async function acceptFulfillmentOrder(
  ...
  if (
    result.errors ||
    result.data?.fulfillmentOrderAcceptFulfillmentRequest?.userErrors?.length >
      0
  ) {
    throw new Error(
      `Failed to accept fulfillment order: ${
        JSON.stringify(
          result.errors ||
            result.data.fulfillmentOrderAcceptFulfillmentRequest.userErrors,
        )
      }`,
    );
  }
...
async function ensureItemExists(
  ...
  if (lineItem.variant_id) {
    ...
    if (variantMatch) {
      ...
      if (
        !variantMatch.shopify_variant_id || !variantMatch.shopify_product_id
      ) {
        await supabase
          .from("items")
          .update({
  ...
  if (lineItem.product_id) {
    ...
    if (productMatch) {
      ...
      // Backfill Shopify IDs if missing
      if (
        !productMatch.shopify_variant_id || !productMatch.shopify_product_id
      ) {
        await supabase
          .from("items")
          .update({
  ...
  console.log(
    `📦 Creating new item for product_id: ${lineItem.product_id}, variant_id: ${lineItem.variant_id}`,
  );

  const sku = sanitizeString(
    lineItem.sku || `SHOP-${lineItem.variant_id || lineItem.product_id}`,
    100,
  );
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (normalizedKind !== "fulfillment_request") {
      ...
      return new Response(
        JSON.stringify({
          message: `Callback kind ${webhook.kind} acknowledged`,
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: store, error: storeError } = await supabase
      .from("shopify_stores")
      .select(
        "id, company_id, store_url, access_token, fulfillment_service_location_id",
      )
    ...
    if (storeError || !store) {
      console.error(
        "No active Shopify store found for shop:",
        shopDomain,
        storeError,
      );
      return new Response(
    ...
    for (const fo of fulfillmentOrders) {
      ...
      if (existing) {
        console.log(
          `⏭️  Fulfillment order ${fulfillmentOrderId} already processed, skipping`,
        );
      ...
      const fulfillmentOrderNumber = fulfillmentOrderId.replace(
        "gid://shopify/FulfillmentOrder/",
        "",
      );

      console.log(
        `🔨 Processing fulfillment order ${fulfillmentOrderNumber} for order ${shopifyOrderNumber}`,
      );
      ...
      for (const lineItemEdge of fo.order.lineItems.edges) {
        ...
        const variantIdNum = lineItem.variant?.id
          ? parseInt(
            lineItem.variant.id.replace("gid://shopify/ProductVariant/", ""),
          )
          : null;
        const productIdNum = lineItem.product?.id
          ? parseInt(lineItem.product.id.replace("gid://shopify/Product/", ""))
          : null;
        ...
        try {
          ...
          if (
            details.shopify_variant_id === variantIdNum?.toString() ||
      ...
      console.log(
        `📊 Item processing: ${itemsMatched} matched, ${itemsCreated} created`,
      );
      ...
      const orderData = {
        ...
        customer_name: sanitizeString(
          `${fo.order.customer?.firstName || ""} ${
            fo.order.customer?.lastName || ""
          }`.trim(),
          255,
        ) || sanitizeString(
          `${fo.destination.firstName || ""} ${fo.destination.lastName || ""}`
            .trim(),
          255,
        ) || "Unknown",
        customer_email: sanitizeString(
          fo.destination.email || fo.order.customer?.email,
          255,
        ),
        customer_phone: sanitizeString(
          fo.destination.phone || fo.order.customer?.phone,
          20,
        ),
        customer_company: sanitizeString(
          fo.destination.company || fo.order.shippingAddress?.company,
          255,
        ),
        shipping_address: {
        ...
        required_delivery_date:
          addBusinessDays(new Date(fo.order.createdAt), 5).toISOString().split(
            "T",
          )[0],
      ...
      const enrichedFulfillmentLineItems = fo.lineItems.edges.map(
        (fliEdge: any) => {
          ...
          return {
            ...
            line_item_id: fli.lineItem.id.replace(
      ...
      try {
        ...
        console.log(
          `✅ Accepted fulfillment order ${fulfillmentOrderId} in Shopify`,
        );
        console.log(
          `   Status: ${acceptedFO.status}, Request Status: ${acceptedFO.requestStatus}`,
        );
        ...
        console.error(
          `⚠️  Failed to accept fulfillment order ${fulfillmentOrderId}:`,
          acceptError,
        );
      ...
      if (elapsed > 4000) {
        console.warn(
          `⏱️  Timeout approaching (${elapsed}ms), stopping processing`,
        );
        break;
      }
    }
    ...
    const elapsed = Date.now() - startTime;
    console.log(
      `✅ Processed ${processedFulfillmentOrderIds.length} fulfillment orders in ${elapsed}ms`,
    );
    ...
    return new Response(
      JSON.stringify({
        ...
        message:
          `Accepted ${processedFulfillmentOrderIds.length} fulfillment order(s)`,
      }),
      {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
