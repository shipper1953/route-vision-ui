...
class EnhancedCartonizationEngine {
  ...
  calculateOptimalBox(items: CartonizationItem[]): any {
    ...
    const totalItemsVolume = items.reduce(
      (sum, item) =>
        sum +
        (item.dimensions.length * item.dimensions.width *
          item.dimensions.height * item.quantity),
      0,
    );
    ...
    console.log(
      `📦 Total items volume: ${totalItemsVolume.toFixed(2)} cubic inches`,
    );
      ...
      if (totalWeight <= box.maxWeight) {
        ...
        if (boxVolume >= totalItemsVolume) {
          ...
          if (canItemsFit) {
            ...
            console.log(
              `✅ Selected box: ${box.name} (${box.length}x${box.width}x${box.height})`,
            );
            ...
            return {
              ...
              shippingCostImpact: this.calculateShippingCostImpact(
                box,
                totalWeight,
              ),
            };
  ...
  private canItemsFitInBox(items: CartonizationItem[], box: Box): boolean {
    ...
    const sortedItems = [...items].sort((a, b) => {
      const volA = a.dimensions.length * a.dimensions.width *
        a.dimensions.height;
      const volB = b.dimensions.length * b.dimensions.width *
        b.dimensions.height;
      return volB - volA;
    });
    ...
    for (const item of sortedItems) {
      ...
      if (
        itemDims[0] > boxDims[0] || itemDims[1] > boxDims[1] ||
        itemDims[2] > boxDims[2]
      ) {
        console.log(
          `❌ Item ${item.name || item.itemId} (${
            itemDims.join("x")
          }) won't fit in box ${box.name} (${boxDims.join("x")})`,
        );
        return false;
      }
    }
  ...
  try {
    ...
    for (const order of orders) {
      try {
        ...
        const cartonizationItems: CartonizationItem[] = orderItems.map(
          (orderItem: any) => {
            const item = itemsMap.get(orderItem.itemId);
            if (!item) {
              console.log(
                `⚠️ Item ${orderItem.itemId} not found in master list`,
              );
              return null;
            }
        ...
        if (cartonizationItems.length === 0) {
          console.log(
            `⚠️ Order ${order.order_id} has no valid items for cartonization`,
          );
          continue;
        }
        ...
        if (result) {
          ...
          if (insertError) {
            console.error(
              `❌ Error saving cartonization for order ${order.order_id}:`,
              insertError,
            );
            errors++;
          } else {
