...
serve(async (req) => {
  ...
  try {
    // SECURITY: require authenticated super_admin
    ...
    // Get warehouse ID
    const warehouseIds = Array.isArray(demoUser.warehouse_ids)
      ? demoUser.warehouse_ids
      : [];
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const customerCompany = Math.random() > 0.3
        ? randomElement(companies)
        : null;
      const customerEmail =
        `${firstName.toLowerCase()}.${lastName.toLowerCase()}@example.com`;
      const customerPhone = `${randomInt(200, 999)}-${randomInt(100, 999)}-${
        randomInt(1000, 9999)
      }`;
    ...
    console.log(
      `Generated ${orders.length} orders, inserting into database...`,
    );
