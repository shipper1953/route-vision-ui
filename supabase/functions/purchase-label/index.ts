...
function createErrorResponse(
  ...
  const clientError = clientMessage ??
    (status === 500
...
async function processWalletPayment(
  ...
  console.log(
    `✅ Deducted $${labelCost.toFixed(2)} from wallet. New balance: $${
...
async function fetchZplContent(
  zplUrl: string,
  apiKey: string,
): Promise<string | null> {
  try {
...
async function ensureValidPhoneNumbers(
  shipmentId: string,
  apiKey: string,
): Promise<void> {
  ...
  if (fromPhone !== sanitizedFromPhone) {
    ...
    console.log(
      `📞 Fixing from_address phone: ${fromPhone} -> ${sanitizedFromPhone}`,
    );
  }
  ...
  if (toPhone !== sanitizedToPhone) {
    ...
    console.log(
      `📞 Fixing to_address phone: ${toPhone} -> ${sanitizedToPhone}`,
    );
  }
...
async function purchaseShippingLabel(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    console.log(
      `📋 Buy request sent with ZPL format request (attempt ${attempt}/${maxAttempts})`,
    );
    ...
    if (!response.ok) {
      ...
      if (responseData.error?.code === "SHIPMENT.POSTAGE.EXISTS") {
        console.log(
          "⚠️ Postage already exists, retrieving existing shipment data",
        );
        try {
          ...
          console.error(
            "❌ Failed to retrieve existing shipment:",
            retrieveError,
          );
        }
      }
      ...
      const isCarrierTimeout =
        responseData.error?.code === "SHIPMENT.POSTAGE.TIMED_OUT" ||
        /timed out/i.test(providerMessage);

      if (isCarrierTimeout && attempt < maxAttempts) {
        const delayMs = attempt * 1500;
        console.warn(
          `⏳ Carrier timeout from EasyPost, retrying in ${delayMs}ms...`,
        );
        await sleep(delayMs);
        continue;
      }
      ...
      if (isCarrierTimeout) {
        throw new Error(
          "Carrier timed out while purchasing this label. Please retry or choose another rate.",
        );
      }
    ...
    if (!zplContent && responseData.postage_label?.label_zpl_url) {
      ...
      zplContent = await fetchZplContent(
        responseData.postage_label.label_zpl_url,
        apiKey,
      );
    }
    ...
    if (zplContent) {
      ...
      console.log(
        "✅ ZPL content successfully retrieved and attached with 4x6 dimensions enforced",
      );
    } else {
      console.warn(
        "⚠️ Could not retrieve ZPL content. This may be due to test mode limitations or carrier restrictions.",
      );
    }
...
async function tryGetZplLabel(
  shipmentId: string,
  apiKey: string,
): Promise<string | null> {
  try {
    console.log(
      "🏷️  Attempting to retrieve ZPL format for shipment:",
      shipmentId,
    );

    // Try to get ZPL from label endpoint with ZPL file format
    const zplResponse = await fetch(
      `https://api.easypost.com/v2/shipments/${shipmentId}/label?file_format=ZPL`,
      {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${apiKey}`,
    ...
    if (zplResponse.ok) {
      ...
      if (zplContent && zplContent.trim().startsWith("^XA")) {
        console.log(
          "✅ Successfully retrieved ZPL format via file_format param (length:",
          zplContent.length,
          ")",
        );
        return zplContent;
    ...
    const acceptResponse = await fetch(
      `https://api.easypost.com/v2/shipments/${shipmentId}/label`,
      {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${apiKey}`,
    ...
    if (acceptResponse.ok) {
      ...
      if (zplContent && zplContent.trim().startsWith("^XA")) {
        console.log(
          "✅ Successfully retrieved ZPL format via Accept header (length:",
          zplContent.length,
          ")",
        );
        return zplContent;
    ...
    console.warn(
      "⚠️  ZPL format not available - this is normal in test mode. Production mode with real carriers will provide ZPL.",
    );
    return null;
  } catch (error) {
...
function ensureZpl4x6Dimensions(zplCode: string): string {
  console.log(
    "🔧 Ensuring ZPL code has 4x6 dimensions (812x1218 dots at 203 DPI)",
  );
...
async function purchaseShippoLabel(
  shipmentId: string,
  rateId: string,
  apiKey: string,
) {
  ...
  console.log(
    "📦 Shippo transaction request body:",
  ...
  try {
    ...
    responseData = {
      ...
      parse_error: getErrorMessage(err),
    };
  }
  ...
  if (responseData.status === "ERROR") {
    const errorMessages = responseData.messages?.map((msg: any) =>
      msg.text
    ).join("; ") || "Unknown error";
  ...
  if (!responseData.label_url) {
    const warningMessages = responseData.messages?.map((msg: any) =>
      msg.text
    ).join("; ") || "No additional details";
    throw new Error(
      `Shippo label was created but no label URL was provided. Messages: ${warningMessages}`,
    );
  }
  ...
  if (
    !zplContent && responseData.label_url &&
    responseData.label_url.includes(".zpl")
  ) {
    ...
    try {
      ...
      if (zplResponse.ok) {
        ...
        if (content && content.trim().startsWith("^XA")) {
          zplContent = content;
          console.log(
            "✅ ZPL content retrieved from Shippo label URL (length:",
            zplContent.length,
            ")",
          );
        }
  ...
  if (zplContent) {
    responseData.label_zpl = ensureZpl4x6Dimensions(zplContent);
    console.log(
      "✅ Shippo ZPL content attached to response with 4x6 dimensions enforced",
    );
  } else {
    console.warn(
      "⚠️ No ZPL content available from Shippo - this may be due to test mode or carrier limitations",
    );
  }
...
async function purchaseEasyshipLabel(
  ...
  if (!shipmentId && shipmentPayload) {
    ...
    console.log(
      "📦 Easyship create body keys:",
      Object.keys(createBody).join(","),
    );
    ...
    if (!createRes.ok) {
      ...
      throw new Error(
        createData.error?.message || createData.message ||
          `Easyship create failed: ${createRes.status}`,
      );
    }
    shipmentId = createData.shipment?.easyship_shipment_id ||
      createData.easyship_shipment_id;
  ...
  const labelRes = await fetch(
    `${easyshipBaseUrl}/2024-09/shipments/${shipmentId}/label`,
    {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${apiKey}`,
  ...
  let labelData: any;
  try {
    labelData = JSON.parse(labelText);
  } catch {
    labelData = { raw: labelText };
  }

  if (!labelRes.ok) {
    console.error("❌ Easyship label purchase failed:", labelData);
    throw new Error(
      labelData.error?.message || labelData.message ||
        `Easyship label failed: ${labelRes.status}`,
    );
  }
  ...
  let trackingNumber: string | undefined = label.tracking_number ||
    shipment.tracking_number;

  if (!labelUrl || !trackingNumber) {
    console.log(
      "⏳ Easyship label not ready inline, polling shipment status...",
    );
    const maxAttempts = 8;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      const pollRes = await fetch(
        `${easyshipBaseUrl}/2024-09/shipments/${shipmentId}`,
        {
          method: "GET",
          headers: {
            "Authorization": `Bearer ${apiKey}`,
      ...
      trackingNumber = polledLabel.tracking_number ||
        polledShipment.tracking_number;

      console.log(
        `🔁 Poll ${attempt}/${maxAttempts} — label_url: ${
          labelUrl ? "ready" : "pending"
        }, tracking: ${trackingNumber ? "ready" : "pending"}, label_state: ${
          polledShipment.label_state || polledLabel.state || "n/a"
        }`,
      );
      ...
      if (labelState === "failed") {
        const failMsg = polledLabel.error_message ||
          polledShipment.label_error || "Easyship label generation failed";
  ...
  console.log(
    "✅ Easyship label purchase complete. Label ready:",
    !!labelUrl,
    "| Tracking:",
...
async function saveShipmentToDatabase(
  ...
  if (!warehouseId) {
    ...
    if (
      userData?.warehouse_ids && Array.isArray(userData.warehouse_ids) &&
      userData.warehouse_ids.length > 0
    ) {
  ...
  if (provider === "easyship") {
    ...
    shipmentData = {
      ...
      service: purchaseResponse.service_name || purchaseResponse.courier_name ||
        "Standard",
      status: "shipped",
      ...
      actual_package_sku: selectedBox?.selectedBoxSku ||
        selectedBox?.selectedBoxName || null,
      actual_package_master_id: selectedBox?.selectedBoxId ||
        selectedBox?.boxId || null,
    };
  } else if (provider === "shippo") {
    const apiCost = parseFloat(
      purchaseResponse.rate?.amount || purchaseResponse.amount || "0",
    );
    if (finalOriginalCost === null || finalOriginalCost === undefined) {
      finalOriginalCost = apiCost;
    }
    ...
    shipmentData = {
      ...
      tracking_number: purchaseResponse.tracking_number,
      carrier: purchaseResponse.rate?.provider ||
        purchaseResponse.carrier_account || "Unknown",
      service: purchaseResponse.rate?.servicelevel?.name ||
        purchaseResponse.servicelevel?.name || "Unknown",
      status: "shipped",
      label_url: purchaseResponse.label_url,
      ...
      estimated_delivery_date: purchaseResponse.rate?.estimated_days
        ? new Date(
          Date.now() +
            purchaseResponse.rate.estimated_days * 24 * 60 * 60 * 1000,
        ).toISOString()
        : null,
    ...
    if (
      !zplContent && purchaseResponse.postage_label?.label_zpl_url && apiKey
    ) {
      console.log("🔄 Fetching ZPL from postage_label.label_zpl_url...");
      zplContent = await fetchZplContent(
        purchaseResponse.postage_label.label_zpl_url,
        apiKey,
      );
    }
    ...
    shipmentData = {
      ...
      estimated_delivery_date: purchaseResponse.tracker?.est_delivery_date ||
        null,
...
async function linkShipmentToOrder(
  ...
  if (!isNaN(Number(orderId))) {
    console.log(
      `📝 Strategy 1: Searching for order with numeric id: ${orderId}`,
    );
  ...
  if (!foundOrder) {
    console.log(
      `📝 Strategy 2: Searching for order with exact order_id: "${orderId}"`,
    );
  ...
  if (!foundOrder) {
    console.log(
      `📝 Strategy 3: Searching for order with case-insensitive order_id: "${orderId}"`,
    );
    const { data: orderByCaseInsensitive, error: searchError3 } =
  ...
  if (foundOrder) {
    console.log(
      `🔄 Processing order ${foundOrder.id} (order_id: ${foundOrder.order_id})`,
    );
    ...
    if (!foundOrder.shipment_id) {
      ...
      console.log(
        `ℹ️ Order already has shipment_id ${foundOrder.shipment_id}, keeping it`,
      );
      orderUpdateSuccess = true;
    }

    console.log(
      `🔗 Creating order_shipments link record for package ${
        packageMetadata?.packageIndex || 0
      }...`,
    );
    ...
    if (enrichedItems.length > 0) {
      try {
        ...
        if (itemIds.length > 0) {
          ...
          if (itemsData) {
            enrichedItems = enrichedItems.map((item: any) => {
              ...
              return {
                ...item,
                shopifyVariantId: itemData?.shopify_variant_gid ||
                  itemData?.shopify_variant_id || item.shopifyVariantId,
              };
            });
        ...
        console.warn(
          "⚠️ Failed to enrich items with Shopify data:",
          enrichError,
        );
    ...
    while (insertAttempts < maxAttempts) {
      ...
      if (!error) {
        console.log(
          `✅ Created order_shipments link record on attempt ${insertAttempts}`,
        );
        break;
      }
    ...
    if (linkError) {
      console.error(
        `❌ All ${maxAttempts} attempts failed to create order_shipments record`,
      );
      orderUpdateSuccess = false;
      throw new Error(
        `Failed to create order_shipments record: ${linkError.message}`,
      );
    }
  } else {
    console.error(
      `❌ Order ${orderId} not found in database using any strategy`,
    );
  }
...
async function recalcOrderFulfillment(supabaseClient: any, orderId: number) {
  try {
    ...
    if (orderErr || !order) {
      console.warn(
        `⚠️ recalcOrderFulfillment: order ${orderId} not found`,
        orderErr,
      );
      return;
    }
    ...
    const itemsTotal = order.items_total
      ...
      : sourceItems.reduce(
        (sum: number, li: any) => sum + Number(li?.quantity || li?.qty || 1),
        0,
      );
    ...
    if (shipErr) {
      console.warn(
        `⚠️ recalcOrderFulfillment: failed loading order_shipments for ${orderId}`,
        shipErr,
      );
      return;
    }
    ...
    for (const row of shipRows || []) {
      const items = Array.isArray(row?.package_info?.items)
        ? row.package_info.items
        : [];
    ...
    const pct = itemsTotal > 0
      ? Math.round((itemsShipped / itemsTotal) * 100)
      : 0;
    ...
    if (updateErr) {
      console.warn(
        `⚠️ recalcOrderFulfillment: failed updating order ${orderId}`,
        updateErr,
      );
    } else {
      console.log(
        `✅ recalcOrderFulfillment: order ${orderId} → ${itemsShipped}/${itemsTotal} (${pct}%) ${fulfillmentStatus}${
...
async function reduceInventoryOnShipment(
  ...
  if (orderError || !orderData) {
    console.warn(
      `⚠️ Unable to load order ${orderId} for shipment inventory deduction`,
      orderError,
    );
    return;
  }

  const sourceItems = shippedItems.length > 0
    ? shippedItems
  ...
  if (qtyBySku.size > 0) {
    ...
    for (const mappedItem of skuItems || []) {
      ...
      if (skuQty > 0) {
        qtyByItemId.set(
          mappedItem.id,
          (qtyByItemId.get(mappedItem.id) || 0) + skuQty,
        );
      }
    }
  }
  ...
  for (const [itemId, shipQty] of qtyByItemId.entries()) {
    ...
    if (levelsError) {
      console.error(
        `❌ Failed loading inventory for shipped item ${itemId}:`,
        levelsError,
      );
      continue;
    }
    ...
    for (const level of levels || []) {
      ...
      const newOnHand = Math.max(
        0,
        (level.quantity_on_hand || 0) - shipFromLevel,
      );
      const deallocateQty = Math.min(
        level.quantity_allocated || 0,
        shipFromLevel,
      );
      const newAllocated = Math.max(
        0,
        (level.quantity_allocated || 0) - deallocateQty,
      );
      ...
      if (updateError) {
        console.error(
          `❌ Failed deducting shipped inventory on level ${level.id}:`,
          updateError,
        );
        continue;
      }
    ...
    if (remainingToShip > 0) {
      console.warn(
        `⚠️ Shipment deduction short for item ${itemId}. Requested ${shipQty}, deducted ${
...
serve(async (req) => {
  ...
  try {
    ...
    if (!shipmentId || !rateId) {
      return createErrorResponse(
        "Missing required parameters: shipmentId and rateId are required",
        null,
        400,
      );
    }
    ...
    const { data: authData, error: authError } = await userSupabase.auth
      .getUser(token);
    ...
    if (authError || !authenticatedUserId) {
      return createErrorResponse(
        "Unauthorized",
        authError?.message || "Unable to resolve authenticated user",
        401,
      );
    }

    console.log(
      `📦 Purchasing label for shipment ${shipmentId} using ${
        provider || "easypost"
      }`,
    );
    console.log(
      "👤 Authenticated user resolved for purchase-label:",
      authenticatedUserId,
    );
    ...
    try {
      if (provider === "shippo") {
        ...
        purchaseResponse = await purchaseShippoLabel(
          shipmentId,
          rateId,
          shippoApiKey,
        );
        ...
        if (!easyshipApiKey) {
          return createErrorResponse(
            "Easyship API key not configured",
            null,
            500,
          );
        }
        ...
        const isSyntheticEasyshipId = !shipmentId ||
        ...
        purchaseResponse = await purchaseEasyshipLabel(
          easyshipShipmentId,
          rateId,
          easyshipApiKey,
        ...
        purchaseResponse = await purchaseShippingLabel(
          shipmentId,
          rateId,
          apiKey,
        );
    ...
    try {
      ...
      if (authUserError || !authUserProfile?.company_id) {
        return createErrorResponse(
          "User profile not found or not assigned to a company",
      ...
      if (orderId) {
        ...
        if (orderRow?.company_id) {
          ...
          if (companyIdForWallet !== authUserProfile.company_id) {
            return createErrorResponse(
              "Unauthorized",
              "Order company does not match authenticated user company",
              403,
            );
          }
        }
      ...
      const carrierCost = provider === "shippo"
        ? parsePositiveAmount(
          purchaseResponse.rate?.amount || purchaseResponse.amount,
        )
        : provider === "easyship"
      ...
      if (
        markedUpLabelCost === null && carrierCost !== null && companyIdForWallet
      ) {
        try {
          ...
          if (companyData?.markup_value && companyData.markup_value > 0) {
            if (companyData.markup_type === "percentage") {
              markedUpLabelCost = carrierCost +
                (carrierCost * companyData.markup_value / 100);
            } else if (companyData.markup_type === "fixed") {
              markedUpLabelCost = carrierCost + companyData.markup_value;
            }
            console.log("📊 Server-side markup applied:", {
              carrierCost,
              markup: companyData.markup_value,
              type: companyData.markup_type,
              markedUpLabelCost,
            });
          }
        } catch (markupErr) {
          console.error(
            "Failed to fetch company markup, using carrier cost:",
            markupErr,
          );
        }
      }
      ...
      if (!companyIdForWallet) {
        return createErrorResponse(
          ...
          "Company not resolved for wallet charge",
          500,
        );
      }
    ...
    try {
      ...
      return createErrorResponse(
        "Failed to save shipment to database",
      ...
      try {
        ...
        if (!enhancedMetadata?.items?.length && selectedItems?.length > 0) {
          ...
          enhancedMetadata = {
            ...
            items: selectedItems,
            boxData: {
              name: selectedBox?.selectedBoxName || selectedBox?.boxName ||
                "Unknown",
        ...
        if (orderData?.company_id) {
          ...
          if (
            shopifyStore?.is_active && shopifyStore?.fulfillment_sync_enabled
          ) {
            ...
            try {
              ...
              console.log(
                "✅ Shopify fulfillment update triggered successfully",
              );
            } catch (shopifyError) {
              console.error(
                "⚠️ Shopify fulfillment update failed (non-fatal):",
                shopifyError,
              );
            }
          }
    ...
    return createSuccessResponse({
      ...
      service: provider === "shippo"
        ...
        : purchaseResponse.selected_rate?.service,
      provider: provider || "easypost",
      labelPending: provider === "easyship"
        ? !!purchaseResponse.label_pending
        : false,
    });
  } catch (error) {
