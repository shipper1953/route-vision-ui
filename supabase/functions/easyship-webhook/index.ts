...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log("📩 Easyship webhook received:", {
      ...
      easyship_shipment_id: event.easyship_shipment_id ||
        event.resource?.easyship_shipment_id,
    });

    const eventType: string = event.event_type || event.type || "";
    const resource = event.resource || event.data || event;

    const easyshipShipmentId = resource.easyship_shipment_id ||
      event.easyship_shipment_id ||
      resource.shipment_id;
    const trackingNumber = resource.tracking_number ||
    ...
    if (easyshipShipmentId) {
      orFilters.push(`easypost_id.eq.${easyshipShipmentId}`);
    }
    ...
    if (!shipments || shipments.length === 0) {
      console.log(
        `No shipment found for easyship_id=${easyshipShipmentId} tracking=${trackingNumber}`,
      );
      return new Response("OK", { status: 200, headers: corsHeaders });
    }
    ...
    const checkpointStatus = (resource.status || resource.tracking_status || "")
      .toLowerCase();

    if (eventType.includes("label.created")) {
      newStatus = "shipped";
    } else if (
      eventType.includes("label.failed") || eventType.includes("cancelled")
    ) {
      newStatus = "cancelled";
    } else if (
      eventType.includes("tracking") || eventType.includes("checkpoint")
    ) {
      if (checkpointStatus.includes("delivered")) {
        newStatus = "delivered";
        actualDeliveryDate = resource.delivered_at || resource.checkpoint_at ||
          new Date().toISOString();
        notificationType = "delivered";
      } else if (
        checkpointStatus.includes("out_for_delivery") ||
        checkpointStatus.includes("out for delivery")
      ) {
        newStatus = "out_for_delivery";
        notificationType = "out_for_delivery";
      } else if (
        checkpointStatus.includes("in_transit") ||
        checkpointStatus.includes("in transit")
      ) {
        newStatus = "in_transit";
      } else if (
        checkpointStatus.includes("exception") ||
        checkpointStatus.includes("failure")
      ) {
        notificationType = "exception";
      }

      if (resource.estimated_delivery_date || resource.eta) {
        estimatedDeliveryDate = resource.estimated_delivery_date ||
          resource.eta;
      }
    }
      ...
      await supabase.from("tracking_events").upsert(
        {
          ...
          carrier_timestamp: resource.checkpoint_at || resource.timestamp ||
            new Date().toISOString(),
    ...
    if (
      ...
      if (estimatedDeliveryDate) {
        orderUpdate.estimated_delivery_date = estimatedDeliveryDate;
      }
      if (actualDeliveryDate) {
        orderUpdate.actual_delivery_date = actualDeliveryDate;
      }
      ...
      if (Object.keys(orderUpdate).length > 0) {
        ...
        for (const jo of junctionOrders || []) {
          await supabase.from("orders").update(orderUpdate).eq(
            "id",
            jo.order_id,
          );
        }
