...
serve(async (req) => {
  ...
  try {
    ...
    if (!shipmentId || Number.isNaN(shipmentId)) {
      return err("shipmentId required", 400);
    }
    ...
    const { data: shipment, error: shipErr } = await admin
      .from("shipments")
      .select(
        "id, easypost_id, company_id, label_url, tracking_number, tracking_url",
      )
      .eq("id", shipmentId)
      .maybeSingle();
    if (shipErr || !shipment) {
      return err("Shipment not found", 404, shipErr?.message);
    }
    if (shipment.company_id !== userRow.company_id) {
    ...
    const res = await fetch(
      `${baseUrl}/2024-09/shipments/${shipment.easypost_id}`,
      {
        headers: {
    ...
    if (!res.ok) {
      return err(
        data?.error?.message || `Easyship API error ${res.status}`,
        502,
        data,
      );
    }
    ...
    const trackingNumber = polledLabel.tracking_number ||
      polledShipment.tracking_number;
    const trackingUrl = polledLabel.tracking_page_url ||
      polledShipment.tracking_page_url;
    ...
    if (labelState === "failed") {
      return new Response(
        JSON.stringify({
          status: "failed",
          message: polledLabel.error_message || polledShipment.label_error ||
