...
Deno.serve(async (req) => {
  ...
  try {
    const { customerEmail, companyId, currentOrderId }: ConsolidationRequest =
      await req.json();
    ...
    eligibleOrders.forEach((order) => {
      const addressKey =
        `${order.shipping_street}-${order.shipping_city}-${order.shipping_zip}`;
    ...
    return new Response(
      JSON.stringify({ error: "Failed to detect opportunities" }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      },
