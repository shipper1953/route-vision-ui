...
serve(async (req) => {
  ...
  try {
    ...
    const { storeUrl, companyId, customerId, customerName, customerReference } =
      await req.json();
    ...
    if (!cleanUrl.includes(".myshopify.com")) {
      throw new Error(
        `Invalid Shopify store URL: "${cleanUrl}". Please use your store's .myshopify.com domain (e.g., your-store.myshopify.com), not an admin URL.`,
      );
    }
    ...
    if (cleanUrl.includes("admin.shopify.com")) {
      throw new Error(
        "Please use your actual store domain (your-store.myshopify.com), not an admin.shopify.com URL",
      );
    }
