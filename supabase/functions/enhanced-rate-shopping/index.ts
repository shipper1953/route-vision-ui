...
serve(async (req) => {
  ...
  try {
    ...
    const totalWeight = shipment_data.packages.reduce(
      (sum, pkg) => sum + pkg.weight,
      0,
    );
    const totalPackages = shipment_data.packages.length;
    const maxDimension = Math.max(
      ...shipment_data.packages.flatMap(
        (pkg) => [pkg.length, pkg.width, pkg.height],
      ),
    );

    console.log(
      `Shipment analysis: ${totalWeight}lbs, ${totalPackages} packages, max dimension: ${maxDimension}"`,
    );
...
async function getFreightRates(shipmentData: ShipmentData) {
  ...
  console.log(
    "Freight integration not yet implemented - returning mock quotes",
  );

  // Mock freight quotes based on total weight and dimensions
  const totalWeight = shipmentData.packages.reduce(
    (sum, pkg) => sum + pkg.weight,
    0,
  );
