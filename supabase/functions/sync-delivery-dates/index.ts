...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: shipments, error: shipmentsError } = await supabase
      .from("shipments")
      .select(
        "id, easypost_id, tracking_number, estimated_delivery_date, actual_delivery_date, status",
      )
    ...
    for (let i = 0; i < (shipments || []).length; i += batchSize) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map((shipment) =>
          processShipment(shipment, easypostApiKey, supabase, updates)
        ),
      );
    ...
    return new Response(
      JSON.stringify({
        message: `Processed ${
          shipments?.length || 0
        } shipments, updated ${updates.length}`,
        updates,
      }),
...
async function processShipment(
  ...
  console.log(
    `Processing shipment ${shipment.id} with EasyPost ID: ${shipment.easypost_id}`,
  );

  const shipmentUrl =
    `https://api.easypost.com/v2/shipments/${shipment.easypost_id}`;
  ...
  if (!shipmentResponse.ok) {
    ...
    console.error(
      `Failed to fetch shipment for ${shipment.easypost_id}: ${shipmentResponse.status} - ${errorText}`,
    );
    return;
  }
  ...
  if (!trackerResponse.ok) {
    ...
    console.error(
      `Failed to fetch tracker for ${shipment.easypost_id}: ${trackerResponse.status} - ${errorText}`,
    );
    return;
  }
  ...
  if (tracker.status === "delivered") {
    ...
    tracker.status === "in_transit" || tracker.status === "out_for_delivery"
  ) {
    status = "shipped";
  }
  ...
  if (
    ...
    for (const order of linkedOrders || []) {
      const { error } = await supabase.from("orders").update(orderUpdate).eq(
        "id",
        order.id,
      );
    ...
    for (const jo of junctionOrders || []) {
      ...
      if (!error) {
        console.log(`✅ Propagated to order ${jo.order_id} via junction`);
      }
    }
