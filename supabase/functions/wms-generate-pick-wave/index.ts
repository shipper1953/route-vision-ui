...
function calculateDistance(loc1: any, loc2: any): number {
  ...
  return Math.abs(aisleA - aisleB) + Math.abs(rackA - rackB) +
    Math.abs(shelfA - shelfB);
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const lastWaveNum = existingWaves?.[0]?.wave_number?.match(/\d+$/)?.[0] ||
      "0";
    const waveNumber = `WAVE-${
      String(parseInt(lastWaveNum) + 1).padStart(4, "0")
    }`;
