...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: poUpdateError } = await supabaseClient.rpc(
    ...
    if (session) {
      ...
      if (existingInventory) {
        await supabaseClient
          .from("inventory_levels")
          .update({
            quantity_on_hand: existingInventory.quantity_on_hand +
              quantity_received,
            quantity_available: existingInventory.quantity_on_hand +
              quantity_received,
          })
